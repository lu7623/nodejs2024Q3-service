...
export class UserController {
  ...
  @Put(':id')
  @Header('content-type', 'application/json')
  update(@Param('id') id: string, @Body() updatePostDto: UpdateUserDto) {
    if (!updatePostDto.newPassword || !updatePostDto.oldPassword) {
      throw new HttpException(Messages.IncorrectData, HttpStatus.BAD_REQUEST);
    }
    const res = this.userService.update(id, updatePostDto);
